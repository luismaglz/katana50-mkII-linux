...
internal sealed partial class AlsaRawMidiConnection : IMidiConnection
{
    ...
    public AlsaRawMidiConnection(string devicePath)
    {
        ...
        _log = new StreamWriter(MidiLogPath, false, Encoding.UTF8) { AutoFlush = false };
        _log.WriteLine($"[{DateTime.Now:HH:mm:ss.fff}] AlsaRawMidiConnection opened: {devicePath}");

        _listenerThread = new Thread(RunListener) { Name = "AlsaMidiListener", IsBackground = true };
    ...
    public Task SendProgramChangeAsync(byte program, CancellationToken cancellationToken = default) =>
        WriteRawLockedAsync([0xC0, program], cancellationToken);

    public Task SendControlChangeAsync(byte control, byte value, CancellationToken cancellationToken = default) =>
        WriteRawLockedAsync([0xB0, control, value], cancellationToken);
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            /* reader didn't exit cleanly — proceed with cleanup anyway */
        }
                ...
                for (var i = 0; i < n; i++)
                {
                    ...
                    else
                    {
                        ...
                        if ((b & 0xF0) == 0xC0)
                        {
                            ...
                            _log.WriteLine(
                                $"[{DateTime.Now:HH:mm:ss.fff}]   -> PC status byte: {b:X2} (channel {b & 0x0F})");
                            awaitingPcByte = true;
                        }
                        else if (b >= 0x80)
                        {
                        ...
                        else if (awaitingPcByte)
                        {
                            ...
                            _log.WriteLine(
                                $"[{DateTime.Now:HH:mm:ss.fff}]   -> PC program byte: {b:X2} (decimal {b}) — firing ProgramChangeReceived");
                            awaitingPcByte = false;
                        ...
                        {
                            _log.WriteLine(
                                $"[{DateTime.Now:HH:mm:ss.fff}]   -> Non-SysEx data (no PC pending): {b:X2}");
                        }
                    }
                    ...
                    {
                        _log.WriteLine(
                            $"[{DateTime.Now:HH:mm:ss.fff}]   -> SysEx frame complete ({frame.Count}B), firing push/reply");
