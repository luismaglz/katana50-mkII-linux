...
internal sealed class NullKatanaSession : IKatanaSession
{
    ...
    public Task<byte> ReadParameterAsync(KatanaParameterDefinition parameter,
    ...
    public Task<byte> WriteParameterAsync(KatanaParameterDefinition parameter, byte value,
        CancellationToken cancellationToken = default)
        => Task.FromResult(value);

    public Task<byte[]> ReadBlockAsync(IReadOnlyList<byte> address, int length,
        CancellationToken cancellationToken = default)
        => Task.FromResult(new byte[length]);

    public Task WriteBlockAsync(IReadOnlyList<byte> address, IReadOnlyList<byte> data,
        CancellationToken cancellationToken = default)
        => Task.CompletedTask;
    ...
    public Task<IReadOnlyDictionary<string, byte>> ReadAllPatchStatesAsync(
        CancellationToken cancellationToken = default)
        => Task.FromResult<IReadOnlyDictionary<string, byte>>(new Dictionary<string, byte>());
}
