...
public class PedalboardItemTypeSelector : IDataTemplate
{
    ...
    public IDataTemplate? OutputTemplate { get; set; }

    public Control? Build(object? data)
    {
        if (data is not PedalboardItemViewModel item) return null;
        var template = item switch
        {
            { IsInput: true } => InputTemplate,
            { IsOutput: true } => OutputTemplate,
            { IsAmp: true } => AmpTemplate,
            _ => PedalTemplate
        };
        return template?.Build(data);
    }

    public bool Match(object? data) => data is PedalboardItemViewModel;
}
