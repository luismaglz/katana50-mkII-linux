...
public sealed class KatanaSession(IMidiTransport midiTransport) : IKatanaSession
{
    ...
    // The amp only responds to RQ1 for the full 2-byte block. The channel code is in byte[1].
    ...
    public async Task<byte?> ReadCurrentChannelByteAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (bytes.Count != 16 || bytes[0] != 0xF0 || bytes[7] != 0x12 || bytes[^1] != 0xF7)
            return null;

        return bytes[13]; // byte[0] is always 0x00; byte[1] is the channel code
    }
