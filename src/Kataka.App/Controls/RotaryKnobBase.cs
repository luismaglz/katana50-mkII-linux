...
public abstract class RotaryKnobBase : Control
{
    ...
    public static readonly StyledProperty<int> MinimumProperty =
        AvaloniaProperty.Register<RotaryKnobBase, int>(nameof(Minimum));

    public static readonly StyledProperty<int> MaximumProperty =
        AvaloniaProperty.Register<RotaryKnobBase, int>(nameof(Maximum), 100);

    public static readonly StyledProperty<int> ValueProperty =
        AvaloniaProperty.Register<RotaryKnobBase, int>(nameof(Value),
    ...
    public static readonly StyledProperty<bool> IsBipolarProperty =
        AvaloniaProperty.Register<RotaryKnobBase, bool>(nameof(IsBipolar));
    ...
    public static readonly StyledProperty<int?> DisplayMinimumProperty =
        AvaloniaProperty.Register<RotaryKnobBase, int?>(nameof(DisplayMinimum));
    ...
    public static readonly StyledProperty<int?> DisplayMaximumProperty =
        AvaloniaProperty.Register<RotaryKnobBase, int?>(nameof(DisplayMaximum));
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             When <see cref="HasDisplayRange" /> is true the raw <see cref="Value" /> is
    ///             linearly interpolated from [Minimum, Maximum] into [DisplayMinimum, DisplayMaximum]
    ///             and that mapped value is shown (with a "+" prefix when positive and the range crosses zero).
    ///         </item>
    ///         <item>
    ///             When <see cref="IsBipolar" /> is true (legacy mode) the value is shown as an
    ///             offset from the midpoint of [Minimum, Maximum].
    ///         </item>
        ...
        get
        {
            if (HasDisplayRange)
            {
                ...
                var displayValue = (int)Math.Round(
                    DisplayMinimum.Value + ((double)(Math.Clamp(Value, Minimum, Maximum) - Minimum) / wireRange *
                                            displayRange));
    ...
    private void UpdateTooltip()
    {
        ...
        if (!string.IsNullOrWhiteSpace(label))
            panel.Children.Add(new TextBlock { Text = label, FontSize = 13, FontWeight = FontWeight.SemiBold });
