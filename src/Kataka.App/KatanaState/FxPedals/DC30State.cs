...
public class DC30State
{
    ...
    public DC30State(bool isMod)
    {
        ...
        ChorusIntensity = new AmpControlState(p[2],
            description: "Adjusts the depth of the chorus effect. Only when TYPE is CHORUS.");
        EchoIntensity = new AmpControlState(p[3],
            description:
            "Adjusts the amount that is returned to the input. A higher value will increase the number of the delay repeats. Only when TYPE is ECHO.");
        EchoVolume = new AmpControlState(p[4],
            description: "Adjusts the volume of the delay sound. Only when TYPE is ECHO.");
        Tone = new AmpControlState(p[5], description: "Adjusts the tone.");
        Output = new AmpControlState(p[6],
            description: "Selects how the direct and effect sounds are output (D/E, D+E, or D→E).");
    }
}
