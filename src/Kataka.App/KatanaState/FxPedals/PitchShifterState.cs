...
public class PitchShifterState
{
    ...
    {
        var p = isMod
            ? KatanaMkIIParameterCatalog.ModPitchShifterParams
            : KatanaMkIIParameterCatalog.FxPitchShifterParams;
        Voice = new AmpControlState(p[0],
            description: "Selects the number of voices for the pitch shift sound (1VOICE or 2VOICE).");
        Ps1Mode = new AmpControlState(p[1], description: "Selection for the PS1 pitch shifter mode.");
        Ps1Pitch = new AmpControlState(p[2],
            description: "Adjusts the amount of pitch shift (the amount of interval) for PS1 in semitone steps.");
        Ps1Fine = new AmpControlState(p[3],
            description:
            "Make fine adjustments to the PS1 interval. The amount of the change in Fine 100 is equivalent to that of the Pitch 1.");
        ...
        Ps2Pitch = new AmpControlState(p[6],
            description: "Adjusts the amount of pitch shift for PS2 in semitone steps.");
