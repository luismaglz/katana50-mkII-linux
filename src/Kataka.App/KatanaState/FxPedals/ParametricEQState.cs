...
public class ParametricEQState
{
    ...
    {
        var p = isMod
            ? KatanaMkIIParameterCatalog.ModParametricEqParams
            : KatanaMkIIParameterCatalog.FxParametricEqParams;
        LowCut = new AmpControlState(p[0],
        ...
        LoMidFreq = new AmpControlState(p[2],
            description: "Specifies the center of the frequency range that will be adjusted by the LO-MID GAIN.");
        LoMidQ = new AmpControlState(p[3],
            description:
            "Adjusts the width of the area affected by the EQ centered at the LO-MID FREQ. Higher values will narrow the area.");
        LoMidGain = new AmpControlState(p[4], description: "Adjusts the low-middle frequency range tone.");
        HiMidFreq = new AmpControlState(p[5],
            description: "Specifies the center of the frequency range that will be adjusted by the HI-MID GAIN.");
        HiMidQ = new AmpControlState(p[6],
            description:
            "Adjusts the width of the area affected by the EQ centered at the HI-MID FREQ. Higher values will narrow the area.");
        ...
        HighCut = new AmpControlState(p[9],
