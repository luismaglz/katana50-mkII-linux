...
public class TWahState
{
    public TWahState(bool isMod)
    {
        ...
        Polarity = new AmpControlState(p[1], description: "Selects the direction in which the filter will change. DOWN cuts the frequency; UP raises the frequency.");
        Sens = new AmpControlState(p[2], description: "Specifies the sensitivity with which the filter changes. Higher values produce a stronger tone which emphasizes the wah effect.");
        Freq = new AmpControlState(p[3], description: "Adjusts the center frequency of the Wah effect.");
        Peak = new AmpControlState(p[4], description: "Adjusts the way in which the wah effect applies to the area around the center frequency.");
