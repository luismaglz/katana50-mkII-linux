...
public class CompState
{
    public CompState(bool isMod)
    {
        ...
        Sustain = new AmpControlState(p[1], description: "Adjusts the range (time) over which low-level signals are boosted. Larger values result in longer sustain.");
        Attack = new AmpControlState(p[2], description: "Adjusts the strength of the picking attack when the strings are played. Higher values result in a sharper attack.");
